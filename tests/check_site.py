...
(async()=>{
 ...
 assert(sms.startsWith("sms:+16788307596&body="),"Monday SMS routes to Matt");
 assert(decodeURIComponent(sms.split("body=")[1])==="I would like to reserve a seat for Hold Em on Monday.","SMS body");
 assert(elements.content.children[1].children[1].children[1].href.startsWith("sms:+16788307596&body="),"Wednesday SMS routes to Matt");
 ...
 api=async(action,fields,session)=>{
   ...
   if(action==="member_settings")return {settings:{reservation_phone_1:"7708615443",reservation_phone_2:"6788307596"}};
